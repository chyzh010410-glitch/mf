<template>
  <div class="page-container">
    ...
    <el-tabs v-model="status" @tab-change="fetchList">
    ...
    <div v-for="order in orders" :key="order.id" style="background:#fff;border-radius:8px;padding:16px;margin-bottom:12px;border:1px solid #ebeef5">
      <div style="display:flex;justify-content:space-between;margin-bottom:12px">
        ...
        <el-tag size="small">待付款</el-tag>
      </div>
      <div style="display:flex;justify-content:space-between;align-items:center">
        <span>共 {{ order.items?.length || 0 }} 件</span>
        <span style="font-size:18px;color:#e74c3c">¥{{ order.payAmount }}</span>
      </div>
...
const orders = ref([]); const total = ref(0); const page = ref(1); const size = ref(10); const status = ref(''); const loading = ref(false)
const fetchList = async () => { loading.value=true; try { const res = await request({ url:'/client/orders',method:'get',params:{page:page.value,size:size.value,status:status.value||undefined} }); if (res.code===200&&res.data) { orders.value=res.data.records||[]; total.value=Number(res.data.total)||0 } } catch {} finally { loading.value=false } }
onMounted(fetchList)
</script>
